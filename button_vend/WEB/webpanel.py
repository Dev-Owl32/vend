        ...
        if ("id" in session):
            if (session["id"] != "423045193902850049"):
                if (request.form["roleid"].isdigit()):
                    ...
                    cur.execute("UPDATE serverinfo SET cultureid = ?, culturepw = ?, logwebhk = ?, buylogwebhk = ?, roleid = ?, culture_fee = ?, bank = ?;", (request.form["cultureid"], request.form["culturepw"], request.form["logwebhk"], request.form["buylogwebhk"], request.form["roleid"], request.form['fee'], json.dumps(bankdata)))
                    con.commit()
                    con.close()
                    return "ok"
                else:
